...
function validateTextBoxPosition(slideData, bodyDimensions) {
	...
	for (const el of slideData.elements) {
		...
		if (
			["p", "h1", "h2", "h3", "h4", "h5", "h6", "list"].includes(el.type)
		) {
...
function addElements(slideData, targetSlide, pres) {
	for (const el of slideData.elements) {
		if (el.type === "image") {
			...
			if (
				el.style.transparency !== null &&
				el.style.transparency !== undefined
			)
...
async function extractSlideData(page) {
	return await page.evaluate(() => {
				...
				if (isText) {
					...
					if (
						...
						if (
							...
							textTransform = (text) =>
								applyTextTransform(text, transformStr);
						}
		...
		document.querySelectorAll("*").forEach((el) => {
			...
			const hasFormatting = el.querySelector(
			...
			if (hasFormatting) {
				...
				if (adjustedStyle.lineSpacing) {
					...
					if (maxFontSize > adjustedStyle.fontSize) {
						...
						adjustedStyle.lineSpacing =
							maxFontSize * lineHeightMultiplier;
					}
				}
